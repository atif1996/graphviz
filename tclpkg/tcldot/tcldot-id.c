...
static int64_t myiddisc_map(void *state, int objtype, char *str, uint64_t *id, int createflag) {
...
static int64_t myiddisc_alloc(void *state, int objtype, uint64_t request_id) {
    NOTUSED(state);
    NOTUSED(objtype);
    NOTUSED(request_id);
    return FALSE;
